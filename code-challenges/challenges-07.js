...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < exparr.length; i++) {
        if (exparr[i].lastName) {
            filteredApp.push({
                fullName: exparr[i].firstName + (" " + exparr[i].lastName),
                tech: exparr[i].tech,
            });
        } else {
            filteredApp.push({
                fullName: exparr[i].firstName,
                tech: exparr[i].tech,
            });
        }
    };
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i].yearsOfExperience <= 1 || ((arr[i].firstName == "" || arr[i].firstName == null) &&( arr[i].lastName == "" || arr[i].lastName == null))) {
